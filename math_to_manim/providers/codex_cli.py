...
class CodexCliProvider:
    ...
    def _build_codegen_prompt(self, spec: ManimSceneSpec) -> str:
        return (
            ...
            "Prefer plain Text or MarkupText labels for formulas unless LaTeX is explicitly requested or confirmed "
            "available; MathTex/Tex scenes require a local LaTeX distribution.\n"
            "IMPORTANT: ALL on-screen text must be in Chinese (Simplified). Use Chinese strings for all Text(), "
            'MarkupText(), and MathTex() calls. For example: use Text("导数") instead of Text("Derivative"). '
            "Equations and formulas can use standard mathematical notation.\n"
    ...
    def _build_repair_prompt(self, spec: ManimSceneSpec, generated: GeneratedCode, failure: str) -> str:
        return (
            ...
            "If the failure mentions missing LaTeX, replace MathTex/Tex formulas with readable plain Text labels.\n"
