...
class AnimationPipeline:
    ...
    def generate(
        ...
        *,
        ...
        # Propagate desired_duration to storyboard and codegen agents
        math_packet_with_duration = math_packet.model_copy(
            update={"metadata": {**math_packet.metadata, "requested_duration_seconds": desired_duration}}
        )

        storyboard = state.put("storyboard", self.storyboard_agent.run(math_packet_with_duration))
        ...
        # Static validation repair loop
        static_repair_attempt = 0
        while (
            not validation.is_successful
            and not self.config.deterministic
            and static_repair_attempt < self.config.max_static_repairs
        ):
            static_repair_attempt += 1
            trace.event(
                "static_repair_requested",
                {
                    "attempt": static_repair_attempt,
                    "scene": generated.scene_name,
                    "issues": [i.model_dump() for i in validation.issues],
                },
            )
            issue_text = "; ".join(i.message for i in validation.issues)
            generated = state.put(
                "generated_code",
                self.codegen_agent.repair(scene_spec, generated, f"Static validation errors: {issue_text}"),
            )
            save_artifact(run_dir, f"generated_code_repair_{static_repair_attempt}", generated)
            trace.event(f"codegen_repair_{static_repair_attempt}", generated.to_public_dict())
            code_path = write_generated_code(generated, run_dir)
            validation = state.put("validation_report", self.static_review_agent.run((generated, code_path)))
            save_artifact(run_dir, f"validation_report_repair_{static_repair_attempt}", validation)

        if render and validation.is_successful:
            from math_to_manim.tools.manim_fixes import fix_manim_common_issues
            ...
            while (
                ...
                generated = state.put(
                    "generated_code",
                    self.codegen_agent.repair(
                        scene_spec, generated, render_result.stderr or render_result.stdout or "render failed"
                    ),
                )
    ...
    def render_existing(run_dir: Path, config: RuntimeConfig | None = None) -> None:
        ...
        generated = GeneratedCode.model_validate(
            json.loads((run_dir / "generated_code.json").read_text(encoding="utf-8"))
        )
