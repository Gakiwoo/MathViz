...
class MathAgent(StageAgent[CurriculumPlan, MathPacket]):
    ...
    def run(self, curriculum: CurriculumPlan) -> MathPacket:
        if not self.config.deterministic:
            artifact = run_structured_sdk_agent(
                ...
                instructions=(
                    ...
                    "assumptions, key equations that can be shown as readable text or Manim MathTex, worked "
                    "examples, common errors, and notes about rendering risks. Keep notation "
                    "minimal and pedagogically sound. "
                    "IMPORTANT: All definitions, examples, explanations, and user-facing text must be in Chinese (Simplified). "
                    "Equations and formulas should use standard mathematical notation (language-agnostic symbols), "
                    "but any accompanying description or label text must be in Chinese."
                ),
        ...
        return MathPacket(
            ...
            definitions=[f"Working definition for {step.title}." for step in steps]
            or [f"Working definition for {target}."],
            ...
            source_notes=["Deterministic seed content; validate formula text/SymPy details before final render."],
